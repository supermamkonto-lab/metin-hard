...
# Liczba prob nawiazania polaczenia z panelem UCP przy timeoucie
_CONNECT_ATTEMPTS = 3


def _log_retry(attempt: int, attempts: int, delay: float) -> None:
    print(f"        Timeout polaczenia (proba {attempt}/{attempts}), ponawiam za {delay:.0f}s...")
...
def login(page: Page, config: AppConfig) -> None:
    ...
    """
    # Krok 1: Nawigacja do panelu UCP (z ponawianiem przy przejsciowym timeoucie)
    try:
        retry_on_timeout(
            lambda: page.goto(config.ucp_url, timeout=config.timeout_ms),
        ...
        raise AuthError(
            f"Panel UCP jest niedostępny: timeout połączenia (po {_CONNECT_ATTEMPTS} próbach)"
        ) from e
